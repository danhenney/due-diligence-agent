...
def run(state: DueDiligenceState, revision_brief: str | None = None) -> dict:
    ...
    if is_public is False:
        data_instructions = (
            ...
            "Instead:\n"
            f"- Search DART filings: web_search('{company} DART 공시') for Korean companies — "
            "DART filings are official and the highest-authority source for Korean company data\n"
            f"- Use web_search for '{company} revenue', '{company} funding round', "
            f"'{company} valuation', '{company} financials'\n"
            "- Prioritize any uploaded documents as primary financial data source\n"
            "- Cross-check uploaded figures against DART/official filings when available\n"
        ...
        data_instructions = (
            ...
            "STEP 2 — DEPTH: Use get_sec_filings for multi-year historical trends (US companies). "
            f"For Korean companies, search DART: web_search('{company} DART 공시 재무제표'). "
            "Cross-check uploaded doc figures against official filings. "
            "Use web_search for additional data.\n\n"
